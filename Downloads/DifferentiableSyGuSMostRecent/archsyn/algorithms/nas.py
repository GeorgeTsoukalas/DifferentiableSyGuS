...
class NAS(ProgramLearningAlgorithm):
            ...
            if valid:
                with torch.no_grad():
                    ...
                    if metric < best_metric:
                        best_metric = metric
                        best_graph = copy.deepcopy(graph)
                    if metric < 1e-4:# performance opitimal threshold
                        break
        ...
        while len(frontier) > 0:
            ...
            for child_id, child_graph in enumerate(children):
                ...
                if terminate:
                    ...
                    print("The cost of " + str(child_graph) + " is " + str(cost))
                else:
                ...
                # store
                if child_cost - cost * penalty < 1e-4:# performance opitimal threshold
                    best_cost = child_cost
                    best_graph = child_graph
                    break
                elif terminate and child_cost < best_cost:
                    best_cost = child_cost
                    best_graph = child_graph
                elif not terminate:
    ...
    # main for execution  ##################
    ###########################################################################################################################
    def run_specific(self, graph, search_loader, train_loader, train_config, device, start_depth=0, warmup=False, train_penalty=0.0, cegis=False, sem="arith"):
        ...
        while cur_iter < len(train_config['specific']):
            log_and_print('Train iteration {} starts \n'.format(cur_iter))
            ...
            if target_topN is not None:
                ...
                if target_topN == 'astar':
                    ...
                    if not graph.check_neural(graph):
                        ...
                        log_and_print('Train iteration {} time spend: {} \n'.format(cur_iter, total_spend))
                        break
                else:
            ...
            if target_depth > cur_depth and not cegis:
                cell_depth = target_depth - cur_depth
                for node in frontier:
                    graph.build_next_cell(node, cell_depth, device, node_share=True, sem=sem)
        ...
        self.search_graph(trained_graph, topN=1, penalty=penalty*train_penalty)
        trained_graph.show_graph()
        log_and_print('\n\n')
        # train and evaluate
        log_and_print('after search \n')
        best_graph = self.train_graph_model(trained_graph, train_loader, train_config, device, lr_decay=lr_decay)
        # calculate time
        total_spend = time.time() - start
        log_and_print('time spend: {} \n'.format(total_spend))

        return best_graph, graph
