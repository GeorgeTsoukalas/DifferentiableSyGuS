...
def lossfxn(out, labels):
        if Testing:
            augmented_out = [(torch.ones(1) - out[l][1]) if labels[l] == 1 else ( out[l][0] if labels[l] == 2 else (torch.ones(1) - (torch.ones(1) - out[l][0])*out[l][1])) for l in range(len(out))] # added implication example here (assuming p-> q atm)
...
def value_correctness(predictions, truths, num_labels=1, threshold=0.5): #Fixme. Num_labels and threshold not important here.
    with torch.no_grad():
        return lossfxn(predictions, truths), None
